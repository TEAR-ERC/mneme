...
namespace detail {
    ...
    template<typename... Ids>
    struct DataLayoutAccessPolicy<DataLayout::AoS, 1u, Ids...> {
    ...
    template<typename... Ids>
    class DataLayoutAllocatePolicy<DataLayout::SoA, Ids...> {
    public:
    ...
    template<typename... Ids>
    struct DataLayoutAccessPolicy<DataLayout::SoA, 1u, Ids...> {
    ...
    struct DataLayoutAccessPolicy<DataLayout::SoA, Extent, Ids...> {
        ...
        constexpr static value_type get(type& c, std::size_t from, std::size_t to) {
            return value_type{ span<typename Ids::type, Extent>(&c.template get<Ids>()[from], to-from)... };
        }
    };
}
...
public:
    ...
    template<std::size_t Extent>
    using value_type = typename access_policy_t<Extent>::value_type;

	MultiStorage(std::size_t size)
        : size_(size) {
    ...
    offset_type offset(std::size_t from) {
        return allocate_policy_t::offset(values, from);
    }
...
class SingleStorage : public MultiStorage<DataLayout::SoA,Id> {
public:
